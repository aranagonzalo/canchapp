...
export async function GET(req: NextRequest) {
    try {
        ...
        const { data: invitaciones, error: invitacionesError } = await db
            ...
            .select("*")
            .in("id_equipo_destinatario", ids_equipos);
        ...
        for (const inv of invitaciones) {
            ...
            const { data: equipoInvitador } = await db
                .from("equipo")
                .select("nombre_equipo, capitan")
                .eq("id_equipo", inv.id_equipo_invitador)
                .single();

            const { data: mailJugador } = await db
                .from("jugador")
                .select("mail")
                .eq("id_jug", equipoInvitador?.capitan)
                .single();
            ...
            resultados.push({
                ...
                id_capitan_invitador: equipoInvitador?.capitan,
                mail_capitan_invitador: mailJugador?.mail,
